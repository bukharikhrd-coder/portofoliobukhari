      ...
      >
        {items.map((item, index) => (
          <motion.div
            ...
            className="group relative flex-shrink-0 w-[160px] sm:w-[200px] md:w-[240px] aspect-square overflow-hidden cursor-pointer bg-secondary/50 rounded-2xl shadow-sm snap-start"
          ...
          >
            ...
            />
            <div className="absolute inset-0 bg-gradient-to-t from-background via-background/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-2xl" />
  ...
  if (loading) {
    return (
      <section id="journey" className="py-20 relative overflow-hidden">
        <div className="container mx-auto px-6 flex items-center justify-center min-h-[200px]">
  ...
  return (
    <section id="journey" className="py-12 md:py-20 relative overflow-hidden">
        ...
        <div className="space-y-8 md:space-y-10">
          {filteredYears.map((year, yearIdx) => (
            <motion.div
              ...
              whileInView={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.6, delay: yearIdx * 0.08, ease: [0.25, 0.1, 0.25, 1] }}
      ...
      {selectedImage && translatedSelected && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 z-50 bg-background/95 backdrop-blur-sm flex items-center justify-center" onClick={closeLightbox}>
