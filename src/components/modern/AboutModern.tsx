...
const AboutModern = () => {
  ...
  return (
    <section id="about" className="py-16 md:py-32 relative overflow-hidden bg-gradient-to-b from-background to-secondary/20">
        ...
        <div className="grid lg:grid-cols-12 gap-8 md:gap-16 items-center" ref={ref}>
          ...
          >
            <div className="relative max-w-xs mx-auto lg:max-w-none">
              ...
              <motion.div
                ...
                className="absolute -top-3 -right-3 w-14 h-14 bg-primary/20 rounded-2xl backdrop-blur-sm flex items-center justify-center text-xl"
              >
                ✨
              </motion.div>
            </div>
          ...
          <div className="lg:col-span-7 space-y-6 md:space-y-8">
            ...
            >
              ...
              <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl mt-4 md:mt-5 leading-tight font-display">
                <TranslatedText>{aboutData.headline_1 || ""}</TranslatedText>
                <br />
                <span className="text-primary"><TranslatedText>{aboutData.headline_2 || ""}</TranslatedText></span>
              </h2>
            </motion.div>
              ...
              <div className="flex flex-wrap gap-2 md:gap-3">
                {translatedSkills.map((skill, index) => (
                  <motion.span
                    ...
                    className="px-3 md:px-4 py-1.5 md:py-2 bg-primary/10 text-primary text-xs md:text-sm rounded-full font-medium hover:bg-primary/20 transition-colors duration-300"
                  >
                    {skill.name}
                  </motion.span>
                ))}
            ...
            >
              {stats.map((stat) => (
                <div key={stat.label} className="text-center bg-card rounded-2xl p-4 md:p-6 shadow-sm border border-border/50">
